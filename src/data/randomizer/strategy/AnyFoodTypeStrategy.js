export const anyFoodTypeStrategy =  
[
  "chandelierBarbecue", "chandelierSteak",
  "chandelierChicken",


  "billboardBarbecue", "billboardSteak",
  "billboardChicken",


  "freightBarbecue", "freightSteak", "freightChicken",


  "dustbinBarbecue", "dustbinSteak", "dustbinChicken",


  "barrelBarbecue", "barrelSteak", "barrelChicken",


  "tireBarbecue", "tireSteak", "tireChicken",


  "telBoothBarbecue", "telBoothSteak",
  "telBoothChicken",


  "drumcanBarbecue", "drumcanSteak", "drumcanChicken",


  "drumcanRollingRightBarbecue",
  "drumcanRollingRightSteak",
  "drumcanRollingRightChicken",


  "drumcanRollingLeftBarbecue",
  "drumcanRollingLeftSteak", "drumcanRollingLeftChicken",


  "drumcanBredKickBarbecue", "drumcanBredKickSteak",
  "drumcanBredKickChicken", 


  "drumcanDugKickBarbecue", "drumcanDugKickSteak",
  "drumcanDugKickChicken",


  "drumcanJakeKickBarbecue", "drumcanJakeKickSteak",
  "drumcanJakeKickChicken",


  "drumcanSimonsKickBarbecue", "drumcanSimonsKickSteak",
  "drumcanSimonsKickChicken",


  // Since lock only allows rolling barrels
  // it is better to increase the probability
  // of normal barrels outside locks.
  "chandelierBarbecue", "chandelierSteak",
  "chandelierChicken",


  "billboard2Barbecue", "billboard2Steak",
  "billboard2Chicken",


  "freightBarbecue", "freightSteak", "freightChicken",


  "dustbinBarbecue", "dustbinSteak", "dustbinChicken",


  "barrelBarbecue", "barrelSteak", "barrelChicken",


  "tireBarbecue", "tireSteak", "tireChicken",


  "telBoothBarbecue", "telBoothSteak",
  "telBoothChicken",


  "drumcanBarbecue", "drumcanSteak", "drumcanChicken",


  // To increase the probability of normal barrels.
  "chandelierBarbecue", "chandelierSteak",
  "chandelierChicken",


  "billboard2Barbecue", "billboardSteak",
  "billboard2Chicken",


  "freightBarbecue", "freightSteak", "freightChicken",


  "dustbinBarbecue", "dustbinSteak", "dustbinChicken",


  "barrelBarbecue", "barrelSteak", "barrelChicken",


  "tireBarbecue", "tireSteak", "tireChicken",


  "telBoothBarbecue", "telBoothSteak",
  "telBoothChicken",


  "drumcanBarbecue", "drumcanSteak", "drumcanChicken",












  "chandelierHamburger", "chandelierHotdog",
  "chandelierPizza", "chandelierCurry",
  "chandelierSushi",


  "billboardHamburger", "billboardHotdog",
  "billboardPizza", "billboardCurry", "billboardSushi",


  "freightHamburger", "freightHotdog", "freightPizza",
  "freightCurry", "freightSushi",


  "dustbinHamburger", "dustbinHotdog", "dustbinPizza",
  "dustbinCurry", "dustbinSushi",


  "barrelHamburger", "barrelHotdog", "barrelPizza",
  "barrelCurry", "barrelSushi",


  "tireHamburger", "tireHotdog", "tirePizza",
  "tireCurry", "tireSushi",


  "telBoothHamburger", "telBoothHotdog",
  "telBoothPizza", "telBoothCurry", "telBoothSushi",


  "drumcanHamburger", "drumcanHotdog", "drumcanPizza",
  "drumcanCurry", "drumcanSushi",


  "drumcanRollingRightHamburger",
  "drumcanRollingRightHotdog",
  "drumcanRollingRightPizza",
  "drumcanRollingRightCurry",
  "drumcanRollingRightSushi",


  "drumcanRollingLeftHamburger",
  "drumcanRollingLeftHotdog",
  "drumcanRollingLeftPizza",
  "drumcanRollingLeftCurry", "drumcanRollingLeftSushi",


  "drumcanBredKickHamburger", "drumcanBredKickHotdog",
  "drumcanBredKickPizza", "drumcanBredKickCurry",
  "drumcanBredKickSushi",


  "drumcanDugKickHamburger", "drumcanDugKickHotdog",
  "drumcanDugKickPizza", "drumcanDugKickCurry",
  "drumcanDugKickSushi",


  "drumcanJakeKickHamburger", "drumcanJakeKickHotdog",
  "drumcanJakeKickPizza", "drumcanJakeKickCurry",
  "drumcanJakeKickSushi",


  "drumcanSimonsKickHamburger",
  "drumcanSimonsKickHotdog", "drumcanSimonsKickPizza",
  "drumcanSimonsKickCurry", "drumcanSimonsKickSushi",


  // Since lock only allows rolling barrels
  // it is better to increase the probability
  // of normal barrels outside locks.
  "chandelierHamburger", "chandelierHotdog",
  "chandelierPizza", "chandelierCurry",
  "chandelierSushi",


  "billboard2Hamburger", "billboard2Hotdog",
  "billboard2Pizza", "billboard2Curry", "billboard2Sushi",


  "freightHamburger", "freightHotdog", "freightPizza",
  "freightCurry", "freightSushi",


  "dustbinHamburger", "dustbinHotdog", "dustbinPizza",
  "dustbinCurry", "dustbinSushi",


  "barrelHamburger", "barrelHotdog", "barrelPizza",
  "barrelCurry", "barrelSushi",


  "tireHamburger", "tireHotdog", "tirePizza",
  "tireCurry", "tireSushi",


  "telBoothHamburger", "telBoothHotdog",
  "telBoothPizza", "telBoothCurry", "telBoothSushi",


  "drumcanHamburger", "drumcanHotdog", "drumcanPizza",
  "drumcanCurry", "drumcanSushi",


  // To increase the probability of normal barrels.
  "chandelierHamburger", "chandelierHotdog",
  "chandelierPizza", "chandelierCurry",
  "chandelierSushi",


  "billboard2Hamburger", "billboardHotdog",
  "billboard2Pizza", "billboardCurry", "billboard2Sushi",


  "freightHamburger", "freightHotdog", "freightPizza",
  "freightCurry", "freightSushi",


  "dustbinHamburger", "dustbinHotdog", "dustbinPizza",
  "dustbinCurry", "dustbinSushi",


  "barrelHamburger", "barrelHotdog", "barrelPizza",
  "barrelCurry", "barrelSushi",


  "tireHamburger", "tireHotdog", "tirePizza",
  "tireCurry", "tireSushi",


  "telBoothHamburger", "telBoothHotdog",
  "telBoothPizza", "telBoothCurry", "telBoothSushi",


  "drumcanHamburger", "drumcanHotdog", "drumcanPizza",
  "drumcanCurry", "drumcanSushi",











  "chandelierBanana", "chandelierPineapple",
  "chandelierApple", "chandelierOrange",
  "chandelierGrape",
  "chandelierRandom3",


  "billboardBanana", "billboardPineapple",
  "billboardApple", "billboardOrange",
  "billboardGrape",
  "billboardRandom3",


  "freightBanana", "freightPineapple", "freightApple",
  "freightOrange", "freightGrape",
  "freightRandom3",


  "dustbinBanana", "dustbinPineapple", "dustbinApple",
  "dustbinOrange", "dustbinGrape",
  "dustbinRandom3",


  "barrelBanana", "barrelPineapple", "barrelApple",
  "barrelOrange", "barrelGrape",
  "barrelRandom3",


  "tireBanana", "tirePineapple", "tireApple",
  "tireOrange", "tireGrape",
  "tireRandom3",


  "telBoothBanana", "telBoothPineapple",
  "telBoothApple", "telBoothOrange", "telBoothGrape",
  "telBoothRandom3",


  "drumcanBanana", "drumcanPineapple", "drumcanApple",
  "drumcanOrange", "drumcanGrape",
  "drumcanRandom3",


  "drumcanRollingRightBanana",
  "drumcanRollingRightPineapple",
  "drumcanRollingRightApple",
  "drumcanRollingRightOrange",
  "drumcanRollingRightGrape",
  "drumcanRollingRightRandom3",


  "drumcanRollingLeftBanana",
  "drumcanRollingLeftPineapple",
  "drumcanRollingLeftApple",
  "drumcanRollingLeftOrange",
  "drumcanRollingLeftGrape",
  "drumcanRollingLeftRandom3",


  "drumcanBredKickBanana", "drumcanBredKickPineapple",
  "drumcanBredKickApple", "drumcanBredKickOrange",
  "drumcanBredKickGrape",
  "drumcanBredKickRandom3",


  "drumcanDugKickBanana", "drumcanDugKickPineapple",
  "drumcanDugKickApple", "drumcanDugKickOrange",
  "drumcanDugKickGrape",
  "drumcanDugKickRandom3",


  "drumcanJakeKickBanana", "drumcanJakeKickPineapple",
  "drumcanJakeKickApple", "drumcanJakeKickOrange",
  "drumcanJakeKickGrape",
  "drumcanJakeKickRandom3",

  "drumcanSimonsKickBanana",
  "drumcanSimonsKickPineapple",
  "drumcanSimonsKickApple", "drumcanSimonsKickOrange",
  "drumcanSimonsKickGrape",
  "drumcanSimonsKickRandom3",


  // Since lock only allows rolling barrels
  // it is better to increase the probability
  // of normal barrels outside locks.
  "chandelierBanana", "chandelierPineapple",
  "chandelierApple", "chandelierOrange",
  "chandelierGrape",
  "chandelierRandom3",


  "billboard2Banana", "billboard2Pineapple",
  "billboard2Apple", "billboard2Orange",
  "billboard2Grape",
  "billboard2Random3",


  "freightBanana", "freightPineapple", "freightApple",
  "freightOrange", "freightGrape",
  "freightRandom3",


  "dustbinBanana", "dustbinPineapple", "dustbinApple",
  "dustbinOrange", "dustbinGrape",
  "dustbinRandom3",


  "barrelBanana", "barrelPineapple", "barrelApple",
  "barrelOrange", "barrelGrape",
  "barrelRandom3",


  "tireBanana", "tirePineapple", "tireApple",
  "tireOrange", "tireGrape",
  "tireRandom3",

  "telBoothBanana", "telBoothPineapple",
  "telBoothApple", "telBoothOrange", "telBoothGrape",
  "telBoothRandom3",


  "drumcanBanana", "drumcanPineapple", "drumcanApple",
  "drumcanOrange", "drumcanGrape",
  "drumcanRandom3",


  // To increase the probability of normal barrels.
  "chandelierBanana", "chandelierPineapple",
  "chandelierApple", "chandelierOrange",
  "chandelierGrape",
  "chandelierRandom3",


  "billboard2Banana", "billboardPineapple",
  "billboard2Apple", "billboardOrange",
  "billboard2Grape",
  "billboardRandom3",


  "freightBanana", "freightPineapple", "freightApple",
  "freightOrange", "freightGrape",
  "freightRandom3",


  "dustbinBanana", "dustbinPineapple", "dustbinApple",
  "dustbinOrange", "dustbinGrape",
  "dustbinRandom3",


  "barrelBanana", "barrelPineapple", "barrelApple",
  "barrelOrange", "barrelGrape",
  "barrelRandom3",


  "tireBanana", "tirePineapple", "tireApple",
  "tireOrange", "tireGrape",
  "tireRandom3",

  "telBoothBanana", "telBoothPineapple",
  "telBoothApple", "telBoothOrange", "telBoothGrape",
  "telBoothRandom3",


  "drumcanBanana", "drumcanPineapple", "drumcanApple",
  "drumcanOrange", "drumcanGrape",
  "drumcanRandom3",









  "chandelierSoftDrink1", "chandelierSoftDrink2",
  "chandelierBeer1", "chandelierBeer2",
  "chandelierWhisky", "chandelierBeer3",
  "chandelierGum1",
  "chandelierRandom4",


  "billboardSoftDrink1", "billboardSoftDrink2",
  "billboardBeer1", "billboardBeer2",
  "billboardWhisky", "billboardBeer3", "billboardGum1",
  "billboardRandom4",


  "freightSoftDrink1", "freightSoftDrink2",
  "freightBeer1", "freightBeer2", "freightWhisky",
  "freightBeer3", "freightGum1",
  "freightRandom4",


  "dustbinSoftDrink1", "dustbinSoftDrink2",
  "dustbinBeer1", "dustbinBeer2", "dustbinWhisky",
  "dustbinBeer3", "dustbinGum1",
  "dustbinRandom4",


  "barrelSoftDrink1", "barrelSoftDrink2",
  "barrelBeer1", "barrelBeer2", "barrelWhisky",
  "barrelBeer3", "barrelGum1",
  "barrelRandom4",


  "tireSoftDrink1", "tireSoftDrink2", "tireBeer1",
  "tireBeer2", "tireWhisky", "tireBeer3", "tireGum1",
  "tireRandom4",


  "telBoothSoftDrink1", "telBoothSoftDrink2",
  "telBoothBeer1", "telBoothBeer2", "telBoothWhisky",
  "telBoothBeer3", "telBoothGum1",
  "telBoothRandom4",


  "drumcanSoftDrink1", "drumcanSoftDrink2",
  "drumcanBeer1", "drumcanBeer2", "drumcanWhisky",
  "drumcanBeer3", "drumcanGum1",
  "drumcanRandom4",


  "drumcanRollingRightSoftDrink1",
  "drumcanRollingRightSoftDrink2",
  "drumcanRollingRightBeer1",
  "drumcanRollingRightBeer2",
  "drumcanRollingRightWhisky",
  "drumcanRollingRightBeer3",
  "drumcanRollingRightGum1",
  "drumcanRollingRightRandom4",


  "drumcanRollingLeftSoftDrink1",
  "drumcanRollingLeftSoftDrink2",
  "drumcanRollingLeftBeer1",
  "drumcanRollingLeftBeer2",
  "drumcanRollingLeftWhisky",
  "drumcanRollingLeftBeer3",
  "drumcanRollingLeftGum1",
  "drumcanRollingLeftRandom4",


  "drumcanBredKickSoftDrink1",
  "drumcanBredKickSoftDrink2",
  "drumcanBredKickBeer1",
  "drumcanBredKickBeer2",
  "drumcanBredKickWhisky",
  "drumcanBredKickBeer3",
  "drumcanBredKickGum1",
  "drumcanBredKickRandom4",


  "drumcanDugKickSoftDrink1",
  "drumcanDugKickSoftDrink2",
  "drumcanDugKickBeer1",
  "drumcanDugKickBeer2",
  "drumcanDugKickWhisky",
  "drumcanDugKickBeer3",
  "drumcanDugKickGum1",
  "drumcanDugKickRandom4",


  "drumcanJakeKickSoftDrink1",
  "drumcanJakeKickSoftDrink2",
  "drumcanJakeKickBeer1",
  "drumcanJakeKickBeer2",
  "drumcanJakeKickWhisky",
  "drumcanJakeKickBeer3",
  "drumcanJakeKickGum1",
  "drumcanJakeKickRandom4",


  "drumcanSimonsKickSoftDrink1",
  "drumcanSimonsKickSoftDrink2",
  "drumcanSimonsKickBeer1",
  "drumcanSimonsKickBeer2",
  "drumcanSimonsKickWhisky",
  "drumcanSimonsKickBeer3",
  "drumcanSimonsKickGum1",
  "drumcanSimonsKickRandom4",


  // Since lock only allows rolling barrels
  // it is better to increase the probability
  // of normal barrels outside locks.
  "chandelierSoftDrink1", "chandelierSoftDrink2",
  "chandelierBeer1", "chandelierBeer2",
  "chandelierWhisky", "chandelierBeer3",
  "chandelierGum1",
  "chandelierRandom4",


  "billboard2SoftDrink1", "billboard2SoftDrink2",
  "billboard2Beer1", "billboard2Beer2",
  "billboard2Whisky", "billboard2Beer3", "billboard2Gum1",
  "billboard2Random4",


  "freightSoftDrink1", "freightSoftDrink2",
  "freightBeer1", "freightBeer2", "freightWhisky",
  "freightBeer3", "freightGum1",
  "freightRandom4",


  "dustbinSoftDrink1", "dustbinSoftDrink2",
  "dustbinBeer1", "dustbinBeer2", "dustbinWhisky",
  "dustbinBeer3", "dustbinGum1",
  "dustbinRandom4",


  "barrelSoftDrink1", "barrelSoftDrink2",
  "barrelBeer1", "barrelBeer2", "barrelWhisky",
  "barrelBeer3", "barrelGum1",
  "barrelRandom4",


  "tireSoftDrink1", "tireSoftDrink2", "tireBeer1",
  "tireBeer2", "tireWhisky", "tireBeer3", "tireGum1",
  "tireRandom4",


  "telBoothSoftDrink1", "telBoothSoftDrink2",
  "telBoothBeer1", "telBoothBeer2", "telBoothWhisky",
  "telBoothBeer3", "telBoothGum1",
  "telBoothRandom4",


  "drumcanSoftDrink1", "drumcanSoftDrink2",
  "drumcanBeer1", "drumcanBeer2", "drumcanWhisky",
  "drumcanBeer3", "drumcanGum1",
  "drumcanRandom4",


  // To increase the probability of normal barrels.
  "chandelierSoftDrink1", "chandelierSoftDrink2",
  "chandelierBeer1", "chandelierBeer2",
  "chandelierWhisky", "chandelierBeer3",
  "chandelierGum1",
  "chandelierRandom4",


  "billboard2SoftDrink1", "billboardSoftDrink2",
  "billboard2Beer1", "billboardBeer2",
  "billboard2Whisky", "billboardBeer3", "billboard2Gum1",
  "billboardRandom4",


  "freightSoftDrink1", "freightSoftDrink2",
  "freightBeer1", "freightBeer2", "freightWhisky",
  "freightBeer3", "freightGum1",
  "freightRandom4",


  "dustbinSoftDrink1", "dustbinSoftDrink2",
  "dustbinBeer1", "dustbinBeer2", "dustbinWhisky",
  "dustbinBeer3", "dustbinGum1",
  "dustbinRandom4",


  "barrelSoftDrink1", "barrelSoftDrink2",
  "barrelBeer1", "barrelBeer2", "barrelWhisky",
  "barrelBeer3", "barrelGum1",
  "barrelRandom4",


  "tireSoftDrink1", "tireSoftDrink2", "tireBeer1",
  "tireBeer2", "tireWhisky", "tireBeer3", "tireGum1",
  "tireRandom4",


  "telBoothSoftDrink1", "telBoothSoftDrink2",
  "telBoothBeer1", "telBoothBeer2", "telBoothWhisky",
  "telBoothBeer3", "telBoothGum1",
  "telBoothRandom4",


  "drumcanSoftDrink1", "drumcanSoftDrink2",
  "drumcanBeer1", "drumcanBeer2", "drumcanWhisky",
  "drumcanBeer3", "drumcanGum1",
  "drumcanRandom4"
];