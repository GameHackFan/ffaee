...
import levelEditorLevelParts from
    "../data/level/LevelEditorLevelParts";    
...
class LevelEditorService
{
  ...
  {
    this.mainData =
        objectUtil.deepCopy(levelDefaultData);
  }
  ...
  applyDataToPatch = (patch) =>
  {
    ...
    Object.keys(this.mainData).forEach((lk) =>
    {
      ...
      Object.keys(level).forEach((egk) =>
      {
        ...
        let leeg = this.getLevelEditorEnemyGroup(
            lk, egk);
        ...
        this.forceEnemy(enemyGroup, leeg,
        ...
        this.fixEnemyGroupEnd(enemyGroup, leeg,
            patch, byteStart);
      });
    });
      ...
      levelEditorEnemyGroup) =>
  {
    ...
    enemy.triggerPosition = this.getValidValue(
        enemy.triggerPosition, leeg.screenPositionStart,leeg.screenPositionEnd);

    if(enemyGroup.mustHaveSpawnDelayTime)
      e.forceTriggerPosition = 1;
  }

  forceEnemy = (enemyGroup, levelEditorEnemyGroup,
      enemyAmount) =>
  {
    if(enemyAmount < 1)
    {
      ...
      enemyId, callback) =>
  {
    ...
    if(!this.isAnythingEmpty([level, lel, leeg]))
    {
      ...
      mergeData = mergeData.concat(
          this.getElevatorMergeData(leeg));
      mergeData = mergeData.concat(
      ...
      levelEditorEnemyGroup) =>
  {
    ...
    mergeData.push(
        {src: glImg, opacity: 0.7, y: py, x: px1});
    mergeData.push(
      {src: glImg, opacity: 0.7, y: py, x: px2});
    return mergeData;
  }
      ...
      enemy, levelEditorEnemyGroup) =>
  {
    ...
    if(enemy && leEnemy)
    {
      ...
      let lh = levelEditorLevelParts
          [leeg.background].height;
      let py = this.getValidValue(enemy.positionY,
          leeg.walkablePositionYBottom, 65535);
  ...
  getElevatorMergeData = (levelEditorEnemyGroup) =>
  {
    ...
    {
      let leeg = levelEditorEnemyGroup;
      let px = this.tryGetField(leeg,
          "levelEditorFOVPositionX", 0);
      ...
      levelEditorEnemyGroup, patch, byteIndex) =>
  {
    ...
    if(leeg.enemyAmountOffset)
    {
      ...
      let eaIndex = leeg.startPosition +
          leeg.enemyAmountOffset;
