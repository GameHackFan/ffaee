...
class ForbiddenService
{
  ...
  getValidEnemyIndexList = (enemyKeyList,
      forbiddenEnemyLists) =>
  {
    let fels = forbiddenEnemyLists ?
        forbiddenEnemyLists : [];
    let fkSet = new Set();
    let veil = [];
  ...
  getValidPositionRangeList = (enemyKey,
      forbiddenRangeList) =>
  {
    let rangeList = objectUtil.deepCopy(
        randomizerData.positionRange);
    let frl = forbiddenRangeList ?
        forbiddenRangeList : [];
    frl.forEach((fk) => {delete rangeList[fk];});

    if(enemyKey.includes("ToFall") ||
        enemyKey.includes("flame"))
    {
      delete rangeList.outsideLeft;
      delete rangeList.outsideRight;
      delete rangeList.inside;
    }
    else if(enemyKey.includes("Falling")
        || enemyKey.includes("chandelier"))
    {
      delete rangeList.outsideLeft;
      delete rangeList.outsideRight;
      delete rangeList.insideTop;
    }
    else if(enemyKey.includes("Kick") ||
        enemyKey.includes("Left"))
    {
      delete rangeList.outsideLeft;
      delete rangeList.inside;
      delete rangeList.insideTop;
    ...
    {
      rangeList.inside =
          randomizerData.positionRange.inside;
    }
